...
| レイアウト | 等分 / メイン左・右・上 / L字 から選択。L字は黒帯が最小になる配置を枠数から自動計算 |
...
<sub>**チャットに書き込みたい場合**は、ブラウザでこのサイトのサードパーティ Cookie を
許可してください（Brave なら Shields を下げる）。埋め込みチャットは iframe なので、
ブロックされているとログイン済みでも投稿できません。読むだけなら設定は不要です。</sub>
